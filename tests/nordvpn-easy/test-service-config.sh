...
assert_eq '15' "$(cat "$(uci_path 'nordvpn_easy.main.wireguard_persistent_keepalive')")" 'WireGuard keepalive backfilled'
assert_eq '' "$(cat "$(uci_path 'nordvpn_easy.main.wireguard_mtu')")" 'WireGuard MTU backfilled as automatic'
assert_eq '1' "$(cat "$(uci_path 'nordvpn_easy.main.firewall_mtu_fix')")" 'firewall MTU fix backfilled'
